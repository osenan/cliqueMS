...
Annotation steps are stored in an `anClique` S3 object. This object can be created from a `XCMSnExp` or a `xcmsSet` object with processed m/z data from `xcms` package. First m/z raw data is processed:
...
mzData <- xcms::findChromPeaks(mzraw, param = cpw)
```
