...
Annotation steps are stored in an `anClique` S3 object. This object can be created from a `xcmsSet` object with processed m/z data from `xcms` package:

```{r anclique}
library(cliqueMS)
ex.anClique <- anClique(exmsSet)
summary(ex.anClique)
```
